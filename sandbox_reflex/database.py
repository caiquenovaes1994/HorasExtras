...
def init_db():
    ...
    with get_db() as conn:
        ...
        if admin_row:
            cur.execute("UPDATE chamados SET username = ? WHERE username IS NULL", (admin_row[0],))
        ...
        if not cur.fetchone():
            cur.execute(
                ...
                "VALUES ('cnovaes', ?, 'CAIQUE NOVAES', TRUE, 'ADMIN', FALSE)",
                (_hash_pw(ADMIN_PWD),)
            )
...
def criar_solicitacao_hotel(rid: str, nome: str, tipo: str, user_id: int):
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO solicitacoes_hoteis (rid, nome, tipo, user_id, status) VALUES (?, ?, ?, ?, 'PENDING')",
...
def processar_solicitacao(sid: int, aprovado: bool):
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute("SELECT rid, nome, tipo FROM solicitacoes_hoteis WHERE id = ?", (sid,))
        row = cur.fetchone()
        if row:
            rid, nome, tipo = row
            if aprovado:
                if tipo == 'CREATE':
                    cur.execute(
                        "INSERT INTO hoteis (rid, nome) VALUES (?, ?) ON CONFLICT (rid) DO UPDATE SET nome = excluded.nome",
                        (rid, nome)
                    )
                elif tipo == 'EDIT':
                    cur.execute("UPDATE hoteis SET nome = ? WHERE rid = ?", (nome, rid))
                elif tipo == 'DELETE':
                    cur.execute("DELETE FROM hoteis WHERE rid = ?", (rid,))
                cur.execute("UPDATE solicitacoes_hoteis SET status = 'APPROVED' WHERE id = ?", (sid,))
            else:
                cur.execute("UPDATE solicitacoes_hoteis SET status = 'REJECTED' WHERE id = ?", (sid,))
...
def registrar_aceite(username: str):
    ...
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute(
            "UPDATE usuarios SET aceitou_termos = TRUE, data_aceite = ? WHERE username = ?",
...
def update_user(uid: int, username: str, nome: str, perfil: str, valor_base: float, password: str | None = None) -> bool:
    try:
        ...
        with get_db() as conn:
            ...
            if password:
                cur.execute(
                    "UPDATE usuarios SET username=?, nome_completo=?, is_admin=?, perfil=?, "
                    "password=?, must_change_password=TRUE, valor_base=? WHERE id=?",
                    (username, nome, is_admin, perfil, _hash_pw(password), _encrypt(valor_base), uid)
                )
            else:
                cur.execute(
                    "UPDATE usuarios SET username=?, nome_completo=?, is_admin=?, perfil=?, valor_base=? WHERE id=?",
...
def update_password(uid: int, new_password: str):
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute("UPDATE usuarios SET password=?, must_change_password=FALSE WHERE id=?", (_hash_pw(new_password), uid))

def reset_password_admin(uid: int):
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute("UPDATE usuarios SET password=?, must_change_password=TRUE WHERE id=?", (_hash_pw("mudar123"), uid))
    ...
    with get_db() as conn:
        ...
        if final_username:
            cur.execute(
                ...
                "FROM chamados WHERE username = ? ORDER BY data ASC, inicio ASC",
                (final_username,)
            )
        else:
...
def update_chamado(cid, data, caso, rid, hotel, inicio, termino, obs, motivo):
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute(
            "UPDATE chamados SET data=?, caso=?, pms=?, hotel=?, inicio=?, termino=?, observacoes=?, motivo=? WHERE id=?",
...
def delete_chamado(cid: int):
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute("DELETE FROM chamados WHERE id = ?", (cid,))
    executar_backup_automatico()
...
def delete_chamados_bulk(cids: list[int]):
    ...
    with get_db() as conn:
        ...
        cur.execute(f"DELETE FROM chamados WHERE id IN ({placeholders})", cids)
    executar_backup_automatico()
